...
Ext.extend(Ext.ux.UploadDialog.Dialog, Ext.Window, {
			...
			"uploading" : {
				...
				"file-upload-success" : [{
					...
					action : [this.resetUploadFrame, this.updateRecordState,
							this.updateProgressBar, this.prepareNextUploadTask,
							this.fireUploadSuccessEvent,this.onCloseButtonClick]
				}, {
					action : [this.resetUploadFrame, this.resetUploadingFlag,
							...
							this.fireUploadCompleteEvent,this.onCloseButtonClick],
					state : "ready"
				}],
				"file-upload-error" : [{
					predicate : this.hasUnuploadedFiles,
	...
	*/},
	...
	prepareNextUploadTask : function() {
		...
		// if (i >= 1) {
		// Ext.ux.Toast.msg('操作提示', '只能上传一个附件！');
		// } else {
		var record = null;
...
//							||
	...
	onAddButtonFileSelected : function(btn) {
		if (this.file_single == 'true') {
			var store = this.grid_panel.getStore();
			var count = store.getCount();
			if (count >= 1) {
				Ext.Msg.alert('操作提示', '只能上传一个附件！');
				return;
			}
		}
	...
	onAjaxSuccess : function(response, options) {
		...
		try {
			// var
			// rsp=response.responseText.replace('<pre>','').replace('</pre>','');
