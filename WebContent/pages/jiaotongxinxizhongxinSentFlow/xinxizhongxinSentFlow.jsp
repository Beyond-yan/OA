...
<h2 align="center" title="公文标题" style="padding: 10px; font-size: 20px;">${prodef.name}</h2>

<span style="float:right;padding-right:7px;padding-bottom:2px;"><a href="<%=request.getContextPath()%>/pages/archivesPrint/sentArchivesPrint.jsp?archiveId=${arch.archivesId}&defId=${defId}" style="text-decoration: none;" target="_blank">进入打印</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="#"  style="text-decoration: none;" onclick="openArchivesEdit('${arch.archivesId}','${fileIds}','${defId}','${prodef.name}','${panelId}')">编辑</a>&nbsp;&nbsp;&nbsp;&nbsp;创建日期:<fmt:formatDate value="${arch.createtime}" pattern="yyyy-MM-dd"/></span>
<table class="table-info" cellpadding="0" cellspacing="1" width="98%" align="center">
 ...
 <tr>
  ...
  <tr>
  	<c:if test="${null!=arch.reviewUserName}">
     <th width="15%">核&nbsp;&nbsp;&nbsp;&nbsp;稿</th>
  ...
  <td colspan="5" style="height:80px;padding-top:5px;" valign="top">
  	<c:forEach var="doc" items="${arch.archivesDocs}">
        ...
        <a href="<%=request.getContextPath()%>/pages/downFile.jsp?fname=<%=pageContext.getAttribute("fileName").toString()%>&fpath=attachFiles/${doc.fileAttach.filePath}" target="_blank">${doc.fileAttach.fileName}</a>&nbsp;&nbsp;&nbsp;&nbsp;
 	 </c:forEach>
  </td>
  </tr>
   <tr>
     ...
     <td colspan="5" style="height:80px;padding-top:5px;" valign="top">
        <c:forEach var="file" items="${arch.archivesFiles}">
	         <a href="<%=request.getContextPath()%>/attachFiles/${file.filePath}" target="_blank">${file.fileName}</a>&nbsp;&nbsp;&nbsp;&nbsp;
		 </c:forEach>    
	 </td>
  </tr>
  <c:if test="${arch.privacyLevel=='一般'}">
...
<%-- 	  </c:if>
  ...
  <%-- <c:if test="${!empty processFormMap}">
	  <c:if test="${processFormMap['综合办公室审核']!=null}">  --%>
		  <tr>
			  <th>综合办公室审核</th>
			  <td colspan="5" ><c:forEach items='${processFormMap["综合办公室审核"]}' var ="form"><ul style="height:10px;padding-top:3px;"><li style="list-style:none;float:left;width:65px;">${form.creatorName}</li><li style="list-style:none;float:left;width:120px"><fmt:formatDate value="${form.createtime}" type="both" pattern="yyyy-MM-dd HH:mm"/></li><li style="list-style:none;float:left">${form.comments}</li></ul><br>
			 </c:forEach></td>
		  </tr>
<%-- 	   </c:if>
  </c:if> --%>
