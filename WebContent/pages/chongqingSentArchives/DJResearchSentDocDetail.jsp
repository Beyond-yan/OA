...
<%@page import="com.gdssoft.oa.service.flow.CcuserProcessService"%>
<%@page import="com.gdssoft.oa.model.flow.CcuserProcess"%>
...
<h2 align="center" title="公文标题" style="padding:10px;font-size:20px;">党建及思想政治工作研究会发文处理笺</h2>
...
<table class="table-info" cellpadding="0" cellspacing="1" width="98%"
	...
	<tr>
		...
		<td colspan="5" style="height: 80px; padding-top: 5px;" valign="top"><c:forEach
				var="doc" items="${arch.archivesDocs}">
  ...
  <c:if test="${!empty processFormMap}">
	  <c:if test="${processFormMap['处室会签']!=null}">
		  <tr>
			  <th>处室会签</th>
			  <td colspan="5" ><c:forEach items='${processFormMap["处室会签"]}' var ="form"><ul style="height:10px;padding-top:3px;"><li style="list-style:none;float:left;width:65px;">${form.creatorName}</li><li style="list-style:none;float:left;width:120px"><fmt:formatDate value="${form.createtime}" type="both" pattern="yyyy-MM-dd HH:mm"/></li><li style="list-style:none;float:left">${form.comments}</li>
			 </ul><br></c:forEach></td>
		  </tr>
	  </c:if>
  </c:if>
