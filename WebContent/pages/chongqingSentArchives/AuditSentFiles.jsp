...
<h1 align="center" style="padding:10px;font-size:20px;">审计处发文处理笺</h1> 
 ...
 <tr>
  ...
  <td colspan="5" style="height:80px;padding-top:5px;" valign="top">
  	<c:forEach var="doc" items="${arch.archivesDocs}">
  	  <c:set var="docName" value="${doc.fileAttach.fileName}"></c:set>
  	   <%   
  	        String url=basePath+"/pages/downFile.jsp?fname="+pageContext.getAttribute("docName").toString()+"&fpath=attachFiles/${doc.fileAttach.filePath}";
      ...
      <a href="<%=url1%>" target="_blank">${doc.fileAttach.fileName}</a>&nbsp;&nbsp;&nbsp;&nbsp;
 	 </c:forEach>
  </td>
  </tr>                                                        
   <tr>
     ...
     <td colspan="5" style="height:80px;padding-top:5px;" valign="top">
        <c:forEach var="file" items="${arch.archivesFiles}">
        	  ...
        	  <% 
				String url2 =response.encodeURL(pageContext.getAttribute("fileName").toString()); 
        	    url2=response.encodeURL(url2);
			  %> 
	         <!--  <a href="<%=request.getContextPath()%>/attachFiles/${file.filePath}" target="_blank">${file.fileName}</a>&nbsp;&nbsp;&nbsp;&nbsp;-->
	         <a href="<%=request.getContextPath()%>/pages/downFile.jsp?fname=<%=url2%>&fpath=attachFiles/${file.filePath}" target="_blank">${file.fileName}</a>&nbsp;&nbsp;&nbsp;&nbsp;
		 </c:forEach>    
	 </td>
  </tr>
  <c:if test="${arch.privacyLevel=='一般'}">
