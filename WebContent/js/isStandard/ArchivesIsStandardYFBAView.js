ArchivesIsStandardYFBAView = Ext.extend(Ext.Panel, {
	// 条件搜索Panel
	searchPanel : null,
	// 数据展示Panel
	gridPanel : null,
	...
	constructor : function(_cfg) {
		...
		// 调用父类构造
		ArchivesIsStandardYFBAView.superclass.constructor.call(this, {
					id : 'ArchivesIsStandardYFBAView',
	...
	initUIComponents : function() {
		...
		this.searchPanel = new Ext.FormPanel({
			height : 88,
			region : 'north',
			id : 'isStandardYFBAsentStoreSearchForm',
			layout : 'form',
			...
			items : [{
				...
				items : [{
							...
							items : [{
										fieldLabel : '标题',
										id : 'isStandardYFBAsentStoredsubject',
										name : 'subject',
										xtype : 'textfield'
									}]
						}, {
							...
							items : [{
								...
								listeners : {
									select : function() {
										Ext
												.getCmp('isStandardYFBAsentStoredStartDt')
												.setValue((this.getValue() + '-01-01'));
										Ext
												.getCmp('isStandardYFBAsentStoredendDate')
							...
							items : [{
										width : 60,
										id : 'isStandardYFBAsentStoredArchivesNo',
										fieldLabel : '发文编号',
										name : 'archiveNo',
										xtype : 'textfield'
									}]
							...
							items : [{
								width : 60,
								id : 'isStandardYFBAsentStoredsnConfigId',
								fieldLabel : '编号办法',
				...
				items : [{
							...
							iconCls : 'reset',
							handler : function() {
								Ext.getCmp('isStandardYFBAsentStoreSearchForm').getForm()
										.reset();
				...
				xtype : 'hidden',
				name : 'endDate',
				id : 'isStandardYFBAsentStoredendDate'
//			}, {
//				xtype : 'hidden',
//				name : 'isStandard',
//				value : 1
		...
		var cm = new Ext.grid.ColumnModel({
			columns : [sm, new Ext.grid.RowNumberer({
						...
						renderer : function(value, metadata, record, rowIndex,
								colIndex) {
							var str = '<a href="#" style="text-decoration:none;color:#0358A3" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#0358A3\'" onclick="ArchivesIsStandardYFBAView.detailT('
	...
	tableToExcel : function(self) {
		...
		if (recordCount > 0) {
			...
			var url = __ctxPath
					...
					+ '&archiveNo='
					+ Ext.getCmp('isStandardYFBAsentStoredArchivesNo').getValue()
					+ '&startDate='
					+ Ext.getCmp('isStandardYFBAsentStoredStartDt')
									.getValue()
					+ '&endDate='
					+Ext.getCmp('isStandardYFBAsentStoredendDate')
									.getValue() 
					+ '&subject='
					+ Ext.getCmp('isStandardYFBAsentStoredsubject').getValue()
//					+'&orgdepId='
//					+Ext.getCmp('orgdepId').getValue()
					+ '&snConfigId='
					+ Ext.getCmp('isStandardYFBAsentStoredsnConfigId').getValue()
//					+ '&issuerName='
//					+ Ext.getCmp('sentIssuerName').getValue()
//					+'&isStandard='
//					+Ext.getCmp('isStandardYFBAsentIsStandard').getValue()
					+'&keywords='
					+Ext.getCmp('isStandardYFBAsentKeywords').getValue();
			url = encodeURI(url);
			url = encodeURI(url);
	...
	onRowAction : function(grid, record, action, row, col) {
		switch (action) {
			case 'btn-archives-detail' :
				ArchivesIsStandardYFBAView.detail(record.data.archiveId);
				break;
			case 'btn-flowView' :
				ArchivesIsStandardYFBAView.toUnStandard(record.data.archiveId);
				break;
			default :
				break;
		}
...
ArchivesIsStandardYFBAView.detail = function(editId) {
	var grid=Ext.getCmp('isStandardBAArchivesGrid');
...
ArchivesIsStandardYFBAView.toUnStandard=function(archivesId){
...
ArchivesIsStandardYFBAView.detailT = function(editId, runId, defId) {
	new ArchDetailView({
				archivesId : editId,
				runId : runId,
				defId : defId,
