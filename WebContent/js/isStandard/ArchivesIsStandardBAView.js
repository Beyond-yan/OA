ArchivesIsStandardBAView = Ext.extend(Ext.Panel, {
	// 条件搜索Panel
	searchPanel : null,
	// 数据展示Panel
	gridPanel : null,
	...
	constructor : function(_cfg) {
		...
		// 调用父类构造
		ArchivesIsStandardBAView.superclass.constructor.call(this, {
					id : 'ArchivesIsStandardBAView',
	...
	initUIComponents : function() {
		...
		this.searchPanel = new Ext.FormPanel({
			height : 88,
			region : 'north',
			id : 'isStandardBASentStoreSearchForm',
			layout : 'form',
			...
			items : [{
				...
				items : [{
							...
							items : [{
										fieldLabel : '标题',
										id : 'isStandardBAsentStoredsubject',
										name : 'subject',
										xtype : 'textfield'
									}]
						}, {
							...
							items : [{
								...
								listeners : {
									select : function() {
										Ext
												.getCmp('isStandardBAsentStoredStartDt')
												.setValue((this.getValue() + '-01-01'));
										Ext
												.getCmp('isStandardBAsentStoredendDate')
							...
							items : [{
										width : 60,
										id : 'isStandardBAsentStoredArchivesNo',
										fieldLabel : '发文编号',
										name : 'archiveNo',
										xtype : 'textfield'
									}]
							...
							items : [{
								width : 60,
								id : 'isStandardBAsentStoredsnConfigId',
								fieldLabel : '编号办法',
				...
				items : [{
							...
							iconCls : 'reset',
							handler : function() {
								Ext.getCmp('isStandardBASentStoreSearchForm').getForm()
										.reset();
				...
				xtype : 'hidden',
				name : 'endDate',
				id : 'isStandardBAsentStoredendDate'
//			}, {
//				xtype : 'hidden',
//				name : 'isStandard',
//				value : 1
		...
		var cm = new Ext.grid.ColumnModel({
			columns : [sm, new Ext.grid.RowNumberer({
						...
						renderer : function(value, metadata, record, rowIndex,
								colIndex) {
							var str = '<a href="#" style="text-decoration:none;color:#0358A3" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#0358A3\'" onclick="ArchivesIsStandardBAView.detailT('
	...
	tableToExcel : function(self) {
		...
		if (recordCount > 0) {
			...
			var url = __ctxPath
					...
					+ '&archiveNo='
					+ Ext.getCmp('isStandardBAsentStoredArchivesNo').getValue()
					+ '&startDate='
					+ Ext.getCmp('isStandardBAsentStoredStartDt')
									.getValue()
					+ '&endDate='
					+Ext.getCmp('isStandardBAsentStoredendDate')
									.getValue() 
					+ '&subject='
					+ Ext.getCmp('isStandardBAsentStoredsubject').getValue()
//					+'&orgdepId='
//					+Ext.getCmp('orgdepId').getValue()
					+ '&snConfigId='
					+ Ext.getCmp('isStandardBAsentStoredsnConfigId').getValue()
//					+ '&issuerName='
//					+ Ext.getCmp('sentIssuerName').getValue()
//					+'&isStandard='
//					+Ext.getCmp('isStandardBAsentIsStandard').getValue()
					+'&keywords='
					+Ext.getCmp('isStandardBAsentKeywords').getValue();
			url = encodeURI(url);
			url = encodeURI(url);
	...
	onRowAction : function(grid, record, action, row, col) {
		switch (action) {
			case 'btn-archives-detail' :
				ArchivesIsStandardBAView.detail(record.data.archiveId);
				break;
			case 'btn-flowView' :
				ArchivesIsStandardBAView.toUnStandard(record.data.archiveId);
				break;
			default :
				break;
		}
...
ArchivesIsStandardBAView.detail = function(editId) {
...
ArchivesIsStandardBAView.toUnStandard=function(archivesId){
...
ArchivesIsStandardBAView.detailT = function(editId, runId, defId) {
	new ArchDetailView({
				archivesId : editId,
				runId : runId,
				defId : defId,
