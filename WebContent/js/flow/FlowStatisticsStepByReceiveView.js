/**
 * 
 * @class FlowStatisticsStepByReceiveView
 * @extends Ext.Panel
 * @description 收文统计报表
 * 
 */
...
FlowStatisticsStepByReceiveView = Ext
		.extend(
				Ext.Panel,
				{
					store : null,
					...
					constructor : function(_cfg) {
						...
						// 调用父类构造
						FlowStatisticsStepByReceiveView.superclass.constructor.call(this,
								{
					...
					initUIComponents : function() {
					 ...
					 proDefStore.on('load',function(store,record,opts){  
						...
						this.store = new Ext.data.JsonStore( {
							proxy: new Ext.data.HttpProxy({ 
								url : __ctxPath + '/flow/getReportStepByReceiveFlowStatisticsReport.do',
					            timeout : 5*1000*60								
					        }),		
							method : 'post',
							root : 'result',
							...
							remoteSort : true,
							autoLoad : false,					
							fields : [ 'recDep','sendDep', 'name','subject',
						...
						function getHtml(store) {
							...
							for(var i=0;i<store.getTotalCount();i++)
							{
								...
								html+="<td width=\'10%\' align=\'left\'>"+store.getAt(i).get("recDep")+"</td>";								
					...
					*/},
					...
					/*alert(self.beginDate);
						...
						if (recordCount > 0) {
							window.location.href =__ctxPath + '/flow/receiveRepStepToExcelDownLoad.do?depName='+depName+'&flowName='+flowName+'&status='+status+'&beginDate='+self.beginDate+'&endDate='+self.endDate+'&staticsDate='+Ext.util.Format.date(new Date(),'Y/m/d')+'&overDateFlag='+self.overDateFlag;
							// 导出Excel
							/*Ext.Ajax.request( {
