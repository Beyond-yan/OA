/**
 * 
 * @class FlowStatisticsSendDepAllStaView
 * @extends Ext.Panel
 * @description 收文统计报表
 * 
 */
...
FlowStatisticsSendDepAllStaView = Ext
		.extend(
				Ext.Panel,
				{
					store : null,
					...
					constructor : function(_cfg) {
						...
						// 调用父类构造
						FlowStatisticsSendDepAllStaView.superclass.constructor.call(this,
								{
					 ...
					 proDefStore.on('load',function(store,record,opts){  
						...
						function getHtml(store) {
							...
							for(var i=0;i<store.getTotalCount();i++)
							{
								...
								html+="<td width=\'10%\' align=\'left\'>"+store.getAt(i).get("subject")+"</td>" +
								...
								"<td width=\'18%\' align=\'left\'>"+store.getAt(i).get("receiveStatus")+"</td>" +
						        "</tr>";
							}
							html = html.replace("'#count'", count.toString());
					...
					*/},
					...
					/*alert(self.beginDate);
						...
						if (recordCount > 0) {
							window.location.href =__ctxPath + '/flow/sendRepToExcelDownLoad.do?depName='+depName+'&flowName='+flowName+'&status='+status+'&beginDate='+self.beginDate+'&endDate='+self.endDate+'&staticsDate='+Ext.util.Format.date(new Date(),'Y/m/d')+'&overDateFlag='+self.overDateFlag;
							// 导出Excel
							/*Ext.Ajax.request( {
