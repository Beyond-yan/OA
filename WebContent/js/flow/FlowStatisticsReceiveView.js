/**
 * 
 * @class FlowStatisticsReceiveView
 * @extends Ext.Panel
 * @description 收文统计报表
 * 
 */
		...
		.extend(
				Ext.Panel,
				{
					...
					constructor : function(_cfg) {
						...
						// 调用父类构造
						FlowStatisticsReceiveView.superclass.constructor.call(this,
								{
									id : 'FlowStatisticsReceiveView',
									title : '收文统计报表明细',
									region : 'center',
									layout : 'border',
					...
					initUIComponents : function() {
					  ...
					  depStore.on('load',function(store,record,opts){  
					     ...
					     if(curUserInfo.isAdmin == true){
							 ;
								...
								Ext.getCmp('receiveReport.depName').setValue(curUserInfo.depName);
								Ext.getCmp('receiveReport.depName').setEditable(false);
								Ext.getCmp('receiveReport.depName').disable();
							}
					   });
					 proDefStore.on('load',function(store,record,opts){  
						...
						function getHtml(store) {
							var beginDate = Ext.util.Format.date(Ext.getCmp('receiveReport.beginDate').getValue(),'Y/m/d');
							var endDate = Ext.util.Format.date(Ext.getCmp('receiveReport.endDate').getValue(),'Y/m/d');
					...
					tableToExcel:function (self) {
						var depName =encodeURI(encodeURI(Ext.getCmp('receiveReport.depName').getValue()));
						var flowName =encodeURI(encodeURI(Ext.getCmp('receiveReport.proDefName').getValue()));
						var status = Ext.getCmp('receiveReport.status').getValue();
						//alert(depName);
						var recordCount = self.store.getCount();
						if (recordCount > 0) {
							window.location.href =__ctxPath + '/flow/receiveRepToExcelDownLoad.do?depName='+depName+'&flowName='+flowName+'&status='+status+'&beginDate='+Ext.util.Format.date(Ext.getCmp('receiveReport.beginDate').getValue(),'Y/m/d')+'&endDate='+Ext.util.Format.date(Ext.getCmp('receiveReport.endDate').getValue(),'Y/m/d')+'&staticsDate='+Ext.util.Format.date(new Date(),'Y/m/d');
							// 导出Excel
							/*Ext.Ajax.request( {
