...
ProcessCCReportView = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		ProcessCCReportView.superclass.constructor.call(this, {
			id : 'ProcessCCReportView',
			title : '阅知给我的流程',
			region : 'center',
			layout : 'border',
		...
		this.gridPanel = new HT.GridPanel( {
			...
			columns : [ {
			...
			}, 
			 {
				...
				dataIndex : 'status',
                renderer:function(value, metadata, record, rowIndex, colIndex){
                    var reHtml = "<span id='spanRead_"+ record.data.runid +"_"+ record.data.ccuserid +"'>";
				...
				renderer:function(value, metadata, record, rowIndex,
					...
					var str = '';
					str+='<a href="#"  onclick="showDetail('
										+ runid + ','+defid+',\''+piid+'\',\''+subject+'\',\''+ record.data.ccuserid +'\')">审批信息</a>';
					if (runid != '0')
                    {
                        if (archivesurl!='')
                        {
                            str+='&nbsp;<a href="#"  onclick="showArchdetailpage(\''+archivesurl+'\',' + runid + ', '+ record.data.ccuserid +')">表单信息</a>';                                            
                        }
                        else
                        {
                            str+='&nbsp;<a href="#"  onclick="showArchdetail(' + runid + ', '+ record.data.ccuserid +')">表单信息</a>';                                            
                        }
                    }
...
//更新为已读
function updateRead(runId, userid){
	Ext.Ajax.request({
        url : __ctxPath + '/flow/updateReadCcuserProcess.do',
        params : {
            processRunId : runId
        },
        method : 'post',
        success : function(res,opt){
           document.getElementById('spanRead_'+runId+'_'+userid).innerHTML = '已读';                       
        },
        failure : function(){
...
function showDetail(runId,defId,piId,name,userid){
	ProcessCCReportView.detail(runId,defId,piId,name);
	updateRead(runId, userid);
}

function showArchdetail(runId,userid){
	ProcessCCReportView.archdetail(runId);
	updateRead(runId, userid);
}

function showArchdetailpage(url,runId,userid){
	ProcessCCReportView.archdetailpage(url,runId);
	updateRead(runId, userid);
}
 ...
 * @param {} name
 */
ProcessCCReportView.detail=function(runId,defId,piId,name){
	new ProcessReportForm({
			runid:runId,
			defid:defId,
			piid:piId,
...
ProcessCCReportView.archdetailpage = function(url,runId){
    var archId = ProcessCCReportView.getArchId(runId);
    if(-1!=archId){
...
ProcessCCReportView.archdetail = function(runId) {
    var archId = ProcessCCReportView.getArchId(runId);
    if(-1!=archId){
...
ProcessCCReportView.getArchId = function(runId){
    var archId;
    Ext.Ajax.request({
