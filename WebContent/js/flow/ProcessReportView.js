...
ProcessReportView = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		ProcessReportView.superclass.constructor.call(this, {
			id : 'ProcessReportView',
			title : '流程综合查询',
	...
	initUIComponents : function() {
		...
		var gridStore = new Ext.data.Store({
		    ...
		    proxy: new Ext.data.HttpProxy({
				...
				'&queryEndDate='+(new Date()).pattern('yyyy/MM/dd'),
		    	timeout:3600000
		    	}),
		    	reader: new Ext.data.JsonReader({
					...
					fields : [ {
						name : 'defid',
						type : 'int'
						},'createtime', 'userid', 'creator',
						'issuedep','subject','name','runid','piid','archivesid','datavalue',
							'runStatus']
				})			
		});
		...
		this.gridPanel = new HT.GridPanel( {
			...
			},
			{
				...
				renderer:function(value, metadata, record, rowIndex,
					...
					var str = '';
					str+='<a href="#"  onclick="ProcessReportView.detail('
										+ runid + ','+defid+',\''+piid+'\',\''+subject+'\')">审批信息</a>';
					if (runid != '0')
					{
						if (archivesurl!=" ")
						{
							str+='&nbsp;<a href="#"  onclick="ProcessReportView.archdetailpage(\''+archivesurl+'\','
							+ runid + ')">表单信息</a>';											
						}
						else
						{
							str+='&nbsp;<a href="#"  onclick="ProcessReportView.archdetail('
							+ runid +','+archivesid+ ')">表单信息</a>';											
						}
					}
					var postUrl = ProRunMap.get(defid);
					//runStatus == 2  流程已经结束   ==3 流程已经终止
					if(defid ==FlowDefIdPro.FilePingShenId_V1){
						
						if((postUrl != "" && postUrl != undefined && (runStatus=='2' || runStatus == '3')) && (defid == FlowDefIdPro.FilePingShenId_V1)){
							//str += '&nbsp;<a href="' + __ctxPath +postUrl+'?runId='+runid+'" target="_blank">打印</a>'+'&nbsp;<a href="' + __ctxPath +postUrl+'?runId='+runid+'" target="_blank">打印</a>';
							str +="<a onClick='ProcessReportView.showWbPsWin(\""+runid+"\")' href='#'>打印</a>";
						}
					}else {
						if (postUrl != "" && postUrl != undefined && (runStatus=='2' || runStatus == '3')&&(defid !=FlowDefIdPro.FeeReimburseId)&&(defid !=FlowDefIdPro.PayExamineId)) {//20121112 xt
							str += '&nbsp;<a href="' + __ctxPath +postUrl+'?runId='+runid+'" target="_blank">打印</a>';
						}						
						if(((defid ==FlowDefIdPro.FeeReimburseId)||(defid ==FlowDefIdPro.PayExamineId))&& (postUrl != "" && postUrl != undefined)){//20121112 xt				
						    str += '&nbsp;<a href="' + __ctxPath +postUrl+'?runId='+runid+'" target="_blank">打印</a>';						
					    }
					}				
 ...
 * @param {} name
 */
ProcessReportView.detail=function(runId,defId,piId,name){
	new ProcessReportForm({
			runid:runId,
			defid:defId,
			piid:piId,
...
// 显示文本评审打印窗口
ProcessReportView.showWbPsWin= function(runid) {
		new WbPsPrinForm( {
			runid : runid
		}).show();
	}
ProcessReportView.archdetailpage = function(url,runId){
   // var archId = ProcessReportView.getArchId(runId);
//    if(-1!=archId){
		var archId;
	    Ext.Ajax.request({
...
ProcessReportView.archdetail = function(editId,archivesid) {
  //  var archId = ProcessReportView.getArchId(editId);
   // if(-1!= archId){
...
ProcessReportView.getArchId = function(runId){	
    var archId;   
    Ext.Ajax.request({
        url : __ctxPath + '/archive/getArchIdArchives.do',
        params : {
