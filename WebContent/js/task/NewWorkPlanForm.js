var NewWorkPlanForm = function(wId, wName) {
	...
	var formPanelBottom = this.setupbottom(wId,wName);
...
NewWorkPlanForm.prototype.setupbottom = function(wId,wName){
	this.planId = wId;
	
	var formPanelBottom = new Ext.FormPanel({
		height : 30,
		frame : false,border:false,
		id : 'NewWorkPlanSaveForm',
		layout : 'hbox',
		layoutConfig: {
            padding:'5',
            align:'middle'
						           ...
						           if (fp.getForm().isValid()) {
									fp.getForm().submit({
												...
												success : function(fp, action) {
													...
													// .reload();
													 //刷新列表窗口
													if(Ext.getCmp('WorkPlanGrid') != null){
													Ext.getCmp('WorkPlanGrid').getStore().reload();
													}
						                            //关闭Panel视窗
													var tabPanel = Ext.getCmp('centerTabPanel');
													tabPanel.remove(Ext.getCmp('NewWorkPlanForm'));
												},
