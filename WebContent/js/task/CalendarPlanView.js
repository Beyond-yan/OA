	...
	var cm = new Ext.grid.ColumnModel({
		columns : [sm, new Ext.grid.RowNumberer(), {
					...
					renderer : function(value, metadata, record, rowIndex,
						...
						var status=record.data.status;
						var assignerId=record.data.assignerId;//分配人
						var userId=record.data.userId;//执行人
						var str='' ;
						if(userId==assignerId){//自己可以删除自己分配的任务
							 str +='<button title="删除" value=" " class="btn-del" onclick="CalendarPlanView.remove('+ editId + ')"></button>';
						}	
						if(status==0){
						...
						str ='<button title="查看" value=" " class="btn-showDetail" onclick="CalendarPlanView.finished('+ editId + ')"></button>';
						}					
						return str;
					}
				}],
