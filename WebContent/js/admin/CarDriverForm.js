 ...
 * @company 捷达世软件
 */
CarDriverForm = Ext.extend(Ext.Window,
{
	//构造函数
	...
	{	
		...
		CarDriverForm.superclass.constructor.call(this,
		{
			id : 'CarDriverFormWin',
			layout : 'fit',
...
//			},
	...
	initUIComponents : function()
	{
			...
			items : [
						{
						 ...
						 {						  	
							...
							border : true,
							height : 500,
							html : '<img src="' + __ctxPath + '/images/default_image_car.jpg" width="300" height="250"/>',
								...
								items : [
									      {
											...
											handler : function() 
											{
												var photo = Ext.getCmp('carDriverImageSource');
										        ...
										        if (photo.getValue() != ''&& photo.getValue() != null&& photo.getValue() != 'undefined') 
										        {
											      ...
											      Ext.Msg.confirm('信息确认', msg + '是否删除？',function(btn) 
												  {
													  ...
													  {
														  // 删除图片
														  var carDriverId = Ext.getCmp('txtCarDriverId').getValue();
														  if (carDriverId != null && carDriverId != 'undefined') 
														  {
																Ext.Ajax.request
																({
																	url : __ctxPath + '/admin/delPhotoCarDriver.do',
																	method : 'post',
																	params : {
																		id : carDriverId
																	},
																	success : function() 
																	{
																		...
																		({
																			...
																			success : function() {dialog.show('queryBtn');}
																		});
		...
		if (this.id != null && this.id != 'undefined')
		{
			this.formPanel.loadData(
			{
				...
				{
					var carimage  = Ext.getCmp('carDriverImageSource').value;
					var carPanel = Ext.getCmp('carImageDisplay');				
		
					if (carimage != null && carimage != '' && carimage != 'undefind'
						&& carPanel.body != null) {
					carPanel.body.update('<img src="' + __ctxPath + '/attachFiles/' + carimage + '"  width="400" height="350"/>');
					}
				}
			});
		}
	...
	save : function()
	{
		$postForm(
		{
			...
			{
				var gridPanel = Ext.getCmp('CarDriverGrid');
				if (gridPanel != null)
				{
