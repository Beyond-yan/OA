OfficeMeetingWleader = Ext.extend(Ext.Panel, {
	// 条件搜索Panel
	searchPanel : null,
	// 数据展示Panel
	gridPanel : null,
	...
	constructor : function(_cfg) {
		...
		// 调用父类构造
		OfficeMeetingWleader.superclass.constructor.call(this, {
					id : 'OfficeMeetingWleader',
	...
	initUIComponents : function() {
		...
		this.searchPanel = new Ext.FormPanel({
					height : 60,
					id:'OfficeMeetingWleaderSearchForm',
					region : 'north',
					layout : 'form',
					...
					items : [{
	                	...
	                	items : [{
							...
							items :[{
								...
								xtype : "hidden",
								name : "timesId",
								id : "OfficeMeetingWleader.timesId"
							}]
	                	},{
							layout:'column',
		                	defaults:{border:false},
								...
								items :[{
									...
									listeners : {
										select : function(cbo, record, index) {
											Ext.getCmp('OfficeMeetingWleader.timesId')
		            	...
		            	items:[{   
							...
							iconCls:'btn-reseted',
							handler:function(){
							var searchPanel = Ext.getCmp('OfficeMeetingWleaderSearchForm');
							...
							id:'sendBgs',
							handler : this.sendBatch.createCallback(this)
						},{
							xtype:'hidden',
							name:'archiveType',
							value:7
		...
		this.store = new Ext.data.JsonStore({
					url : __ctxPath + "/flow/OfficeMeetingWleaderlistFlowTaskReport.do",
					baseParams : {},
					root : 'result',
...
//							var str = '<a href="#" style="text-decoration:none;color:#0358A3" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#0358A3\'" onclick="OfficeMeetingWleader.detail('+record.data.archiveId+','+record.data.runId+','+record.data.defId+')">'+value+'</a>';
//							return str;
							var str = '<a href="#"  title="'+value+'"'+' style="text-decoration:none;color:#0358A3" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#0358A3\'" onclick="OfficeMeetingWleader.detail('+record.data.ARCHIVESID+','+record.data.PROCESS_INS_ID+','+record.data.DEFID+')">'+value+'</a>';
							return str;
						}
					},{
						header : '提案处室',
						...
						renderer : function(value, metadata, record, rowIndex,colIndex) {
							...
							if(record.data.STATUS != 2){
								str += '<a href="#" style="text-decoration:none;color:#3D3D3D" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#3D3D3D\'" onclick="OfficeMeetingWleader.nextStep('
	...
	addTimes : function(self) {
		new OfficeMeetingWleaderAddForm();
	},
	
	setTimes : function(self){
		var grid = Ext.getCmp("OfficeMeetingWleaderGrid");
		var archivesIds=""
		 if (grid.getSelectionModel().hasSelection()){
			   ...
			   new OfficeMeetingWleaderSetForm(archivesIds.substring(0,archivesIds.length-1));
		 }else{
			 Ext.MessageBox.show({
					title : '操作信息',
					msg : '请选择一条记录！',
	...
	sendBatch : function(self){
		var grid = Ext.getCmp("OfficeMeetingWleaderGrid");
		if (grid.getSelectionModel().hasSelection()){
			var records=grid.getSelectionModel().getSelections();
			var archives = new Array();
			for(var i=0; i < records.length; i++){
			   if(records[i].data.CURRENTSTEP != 6){
				   Ext.MessageBox.show({
						title : '操作信息',
						msg : '请选择议题状态为"委领导审批"的记录进行发送！',
						buttons : Ext.MessageBox.OK,
						icon : 'ext-mb-error'
					});
				   return;
			...
			new OfficeMeetingWleaderSendForm(archives);
		 }else{
			 Ext.MessageBox.show({
					title : '操作信息',
					msg : '请选择一条记录！',
	...
	onRowAction : function(grid, record, action, row, col) {
		switch (action) {
			case 'btn-flowView' :
				OfficeMeetingWleader.proDetail(record.data.runId,record.data.defId,record.data.piid,record.data.runSubject);
				break;
			case 'menu-public-fol' :
				OfficeMeetingWleader.nextStep(record.data.taskId,record.data.activityname,record.data.taskName);
				break;
			case 'btn-archives-detail' :
				OfficeMeetingWleader.detail(record.data.archiveId,record.data.runId,record.data.defId);
				break;
			case 'menu-archive-draft':
				OfficeMeetingWleader.writeOpinion(record.data.runId,record.data.defId,record.data.piid,record.data.runSubject);
				break;
			default :
				break;
		}
 ...
 * @param {}
 */
OfficeMeetingWleader.openOfficeMeetingEditWindow = function(archiveId,defId) {
...
OfficeMeetingWleader.detail = function(editId,runId,defId) {
	var isGranted=false;
...
OfficeMeetingWleader.setTimesSingle = function(archivesId) {
	new OfficeMeetingWleaderSetForm(archivesId);
}

OfficeMeetingWleader.setNumberSingle = function(archivesId) {
 ...
 * 
 */
OfficeMeetingWleader.proDetail = function(runId, defId, piId, name) {
...
OfficeMeetingWleader.stopProcess = function(editId, piId) {
				...
				if (btn == 'yes') {
					...
					Ext.Ajax.request({
								...
								success : function(resp, options) {
									...
									Ext.getCmp('OfficeMeetingWleaderGrid')
											.getStore().reload()
								},
 ...
 * @param {}
 */
OfficeMeetingWleader.writeOpinion = function(runId, defId, piId, name) {
 ...
 * @param {} taskdbid
 */
OfficeMeetingWleader.nextStep=function(taskId,runId,activityName,taskName){
	var contentPanel=App.getContentPanel();
...
function refeshTaskPanelView3(){
 	setTimeout(function(){
 		var grid = Ext.getCmp('OfficeMeetingWleaderGrid'); // auditwuGrid 是grid 的id                                         
 		grid.store.reload();
 		refeshTaskPanelView3();
	},60*1000);
}
...
function sendBGSForeach(records, taskIds, index, max){
	...
	var form=Ext.getCmp('OfficeMeetingWleaderSearchForm').getForm();
	var params = {
