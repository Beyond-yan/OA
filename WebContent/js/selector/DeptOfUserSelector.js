...
var departmentUrl = "/system/listDepartment.do";
if(this.depId != null && this.depId != "")
	departmentUrl =  '/system/listByDepIdDepartment.do?depId='+ this.depId;
	...
	clickRoleNode : function(node) {
		if (node != null) {
			...
			store.proxy.conn.url =__ctxPath + '/system/findByRoleInDepAppUser.do';
			store.load({
					params : {
						start : 0,
						limit : 12
	...
	clickTeamNode : function(node) {
		if (node != null) {
			...
			store.proxy.conn.url =__ctxPath + '/system/listMembersAppTeam.do';
			store.load({
					params : {
						start : 0,
						limit : 12
