...
var departmentUrl = "/system/listDepartment.do?flag=msg";
if(this.depId != null && this.depId != "")
	departmentUrl =  '/system/listByDepIdDepartment.do?flag=msg&depId='+ this.depId;
		...
		var selfDefPanel = new Ext.tree.TreePanel({
					id : 'selfDefPanel',
					autoScroll:true,
					iconCls:'role-user',
					title : '按群组分类 ',
					loader : new Ext.tree.TreeLoader({
								url : __ctxPath + '/system/treeAppTeam.do'
							}),
					root : new Ext.tree.AsyncTreeNode({
								expanded : true
							}),
						...
						},
						{
							...
							handler:function(){
								searchPanel.getForm().submit({
									...
									url:__ctxPath+'/system/selectInDepAppUser.do?depId='+ depId,
									method:'post',
		...
		var contactPanel = new Ext.Panel({
					...
					items : [searchPanel,{
								...
								width : 160,
								layout : 'accordion',
								items : [treePanel,rolePanel,selfDefPanel,onlinePanel]
							}, contactGrid]
		});
		if(!isSingle){//非单用户的
	...
	clickNode : function(node) {
		if (node != null) {
			...
			store.proxy =new Ext.data.HttpProxy({url:__ctxPath + '/system/selectAppUser.do'});
			store.baseParams = {
				depId : node.id
			};
			store.load({
	...
	clickRoleNode : function(node) {
		if (node != null) {
			...
			store.proxy =new Ext.data.HttpProxy({url:__ctxPath + '/system/findByRoleInDepAppUser.do'});
			store.load({
					params : {
						start : 0,
						limit : 12
	...
	clickTeamNode : function(node) {
		if (node != null) {
			...
			store.proxy =new Ext.data.HttpProxy({url:__ctxPath + '/system/listMembersAppTeam.do'});
			store.load({
					params : {
						start : 0,
						limit : 12
	...
	clickOnlinePanel:function(){
		var users = Ext.getCmp('contactGrid');
		var store = users.getStore();
		store.proxy =new Ext.data.HttpProxy({url:__ctxPath + '/system/onlineAppUser.do'});
		store.load({
				params : {
					start : 0,
					limit : 200
