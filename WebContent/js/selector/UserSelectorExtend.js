...
var UserSelectorExtend = {
	 ...
	 * @return {}
	 */
	getView : function(callbackOrConf,isSingle,isForFlow,userIds,userNames) {
		//单选
		userids = userIds;
		usernames = userNames;
		...
		var window = new Ext.Window({
			...
			buttons : [{
						...
						handler : function(){
							...
							if(isSingle){//选择单个用户
								var grid = Ext.getCmp('contactGridExt');
								...
								var selStore=Ext.getCmp('selectedUserGridExt').getStore();
	...
	initPanel : function(isSingle) {
		var arruids = userids.split(",");
		var arrunames = usernames.split(",");
		var arrayStore = new Array();
		for (i=0;i<arruids.length;i++)
		{
			if (arruids[i]!='' && arrunames[i]!='')
			{
				arrayStore[i] = new Array(arruids[i],arrunames[i]);				
			}
		}
		var selectedUserStore = new Ext.data.ArrayStore({
			fields: ['userId', 'fullname']			
		});
		selectedUserStore.loadData(arrayStore);
		...
		var treePanel = new Ext.tree.TreePanel({
					...
					loader : new Ext.tree.TreeLoader({
								url : __ctxPath + '/system/listnotrootDepartment.do?isRestAdmin='+isRestAdmin+'&opt=1&depId='+curUserInfo.depId
								//过滤除了当前登录用户所在部门以外的其它部门
							}),
		...
		contactGrid.on('rowdblclick',function(grid,rowIndex,e){
				var contactGrid = Ext.getCmp('contactGridExt');
				var selGrid=Ext.getCmp('selectedUserGridExt');
		...
		var selectedUserGrid = new Ext.grid.EditorGridPanel({
			id : 'selectedUserGridExt',
			title:'已选用户',
			height : 400,
			width:165,
			autoScroll:true,
			store : selectedUserStore,
			trackMouseOver : true,
			sm:csm,
			columns:[
					csm,
	...
	clickNode : function(node) {
		if (node != null) {
			var users = Ext.getCmp('contactGridExt');
