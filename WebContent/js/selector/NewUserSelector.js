...
var NewUserSelector = {
	 ...
	 */
	getView : function(callbackOrConf,isSingle,isForFlow,isSchema) {
		//单选
		var callback=null;
		...
		var panel=this.initPanel(isSingle,isSchema);
	...
	initPanel : function(isSingle,isSchema) {
		...
		var schemaUrl = '/system/findBySchemaDepartment.do?depId=' + roleMap.get('DepartmentCommonId');
	...
	clickRoleNode : function(node) {
		...
		users.reconfigure(new Ext.data.Store({
					proxy : new Ext.data.HttpProxy({
								url : __ctxPath + '/system/findAppUser.do'
							}),
					baseParams: {
						roleId : node.id
					},
