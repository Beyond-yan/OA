	...
	initPanel : function(isSingle) {
		...
		var store = new Ext.data.Store( {
			proxy : new Ext.data.HttpProxy( {
				url : __ctxPath
						+ '/operations/getPersonProApply.do'
			}),
											...
											.submit(
													{
														...
														success : function(
																...
																action) {
															contactGrid
																	.getStore().proxy.conn.url = __ctxPath + '/system/listAppUser.do';
										...
										items : [
												{
													...
													handler : function() {
														...
														var selStore = selGrid
																.getStore();
														var rows = contactGrid
																.getSelectionModel()
																.getSelections();
														for ( var i = 0; i < rows.length; i++) {
															...
															var isExist = false;
															// 查找是否存在该记录
															for ( var j = 0; j < selStore
																	.getCount(); j++) {
																if (selStore
																		.getAt(j).data.userId == userId) {
																	isExist = true;
																	break;
																}
															}
															if (!isExist) {
																...
																var newRecord = new selStore.recordType(
																		newData);
																selGrid
																		.stopEditing();
																selStore
