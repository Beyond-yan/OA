...
ProviderView = Ext.extend(Ext.Panel, {
	...
	initUIComponents : function() {
		...
		/*
		 * this.topbar.add(new Ext.Button({ xtype:'button', text:'发送邮件',
		 * iconCls:'btn-mail_send', handler:function(){ var grid =
		 * Ext.getCmp("ProviderGrid");
		 * 
		 * var selectRecords = grid.getSelectionModel() .getSelections();
		 * 
		 * if (selectRecords.length == 0) { Ext.ux.Toast.msg("信息",
		 * "请选择要发送邮件的供应商！"); return; } var ids = Array(); var names=Array(); for
		 * (var i = 0; i < selectRecords.length; i++) {
		 * ids.push(selectRecords[i].data.providerId);
		 * names.push(selectRecords[i].data.providerName) } new SendMailForm({
		 * ids : ids, names : names, type : 1 }).show(); } } ));
		 */// end of the topbar
...
ProviderView.remove = function(id) {
	...
	Ext.Msg.confirm('信息确认',
			'删除供应商，则该供应商下的<font color="red">产品</font>也删除，您确认要删除该记录吗？',
			function(btn) {
				if (btn == 'yes') {
					Ext.Ajax.request({
								url : __ctxPath
