...
Ext.calendar.EventEditWindow = function(config) {
    ...
    var categoryStore = new Ext.data.JsonStore({
    	...
    	url : '../../system/listByNationalHolidayConfigSysConfig.do',
    	fields:[{
    		name:'configId',
    		type:'int'
    ...
    categoryStore.on("load", function() {
    	if(Ext.getCmp('calendar').getValue()==3){
    		Ext.getCmp('category').setValue(Ext.getCmp('categoryTemp').getValue());
        	Ext.getCmp('category').show();
        }
    });
    var dayCategoryData = [[1, '工作日'], [2, '周休日'], [3, '国定假日'], [4, '补假日']];
    var dayCategoryStore=new Ext.data.SimpleStore({
    	fields : ['value', 'text'],
    ...
    if (config.calendarStore) {
        ...
        formPanelCfg.items.push(/*{
            ...
            listeners: {'select':function(){
				var calendar=Ext.getCmp('calendar').getValue();
            	if(calendar==3){
            		Ext.getCmp('category').show();
				}else{
            ...
            xtype:'label',
            id : 'showHtml',
            html:'<p style="color:#ff0000">本月已存在班表。</p>',
            hidden:true,
            anchor: '85%'
        });
    }
...
//        this);
    ...
    show: function(o, animateTarget) {
        ...
        if(this.isAdd==false){
        		...
        		params:{
        		 ...
        		 success : function(response, options){
        	         ...
        	         if(data.category=="null"){
        	        	 ...
        	        	 Ext.getCmp('categoryTemp').setValue("");
        	         }else{
        	        	 Ext.getCmp('category').setValue(data.category);
        	        	 Ext.getCmp('categoryTemp').setValue(data.category);
        	         }
        	         Ext.getCmp('calendar').setValue(data.dayCategory);
        	         
        	         //Ext.getCmp('category').setValue(rec.data.ConfigKey);
            ...
            Ext.Ajax.request({
	            url : '../../arrange/ifExistAttendCompanyCalendar.do',
                params:{
                    day:rec.data.StartDate
                },
	            success : function(response, options){
	                ...
	                var msg = data.msg;
	                if(true == msg){
                        Ext.getCmp('showHtml').show();
                        Ext.getCmp('showSave').hide();
	                }else{
                        Ext.getCmp('showHtml').hide();
                        Ext.getCmp('showSave').show();
	                    //Ext.Msg.alert("提示信息", "本月已经生成班表，不能对行事历再做修改。");
	                }
	           },
	           failure : function(response) {
	                Ext.Msg.alert("操作信息", "修改失败！请联系管理员。");
	           }
            });
        }
...
//    },
    updateRecord: function() {
        ...
        M = Ext.calendar.EventMappings;
        var title="";
        var dayCategory=Ext.getCmp('calendar').getValue();
    	var category=Ext.getCmp('category').getRawValue();
        if(dayCategory==3){
        	title=category;
        }else if(dayCategory==1){
        	title='工作日';
        }else if(dayCategory==2){
        	title='周休日';
        }else if(dayCategory==4){
        	title='补假日';
        }
        f.updateRecord(this.activeRecord);
        var category=this.formPanel.form.findField('category').getValue();
        var categoryNo="";
        if(dayCategory==3){
	        if(category=="newYear"){
	        	categoryNo="1";
	        }else if(category=="lunarNewYear"){
	        	categoryNo="2";
	        }else if(category=="qingMing"){
	        	categoryNo="3";
	        }else if(category=="mayDay"){
	        	categoryNo="4";
	        }else if(category=="dragonBoat"){
	        	categoryNo="5";
	        }else if(category=="midAutumn"){
	        	categoryNo="6";
	        }else if(category=="nationalDay"){
	        	categoryNo="7";
	        }
        }
        var cid=dayCategory+""+categoryNo;
        this.activeRecord.set(M.StartDate.name, dates[0]);
        this.activeRecord.set(M.EndDate.name, dates[0]);
        this.activeRecord.set(M.Title.name, title);
        this.activeRecord.set(M.IsAllDay.name, true);
        this.activeRecord.set(M.CalendarId.name, eval(cid));
        this.fireEvent(this.isAdd ? 'eventadd': 'eventupdate', this, this.activeRecord,this.activeRecord.get(M.CalendarId.name),title);
    },
    // private
//    onSave: function() {
...
//    },
    onSave: function() {
    	var day=Ext.getCmp('date-range').getValue();
    	var dayCategory=Ext.getCmp('calendar').getValue();
    	var category=Ext.getCmp('category').getValue();
    	if(dayCategory==3&&(category==null||category=="")){
    		Ext.Msg.alert("错误","请选择节假日类型!");
    		return false;
    	}    	
    	Ext.Ajax.request({
            url : '../../arrange/saveDayAttendCompanyCalendar.do',
            params:{
                day:day,
...
//    		 success : function(response, options){
//    			 Ext.Ajax.request({
//    					url : '../../arrange/searchListAttendCompanyCalendar.do',
//    					 success : function(response, options){
//    						 var data = response.responseText;
//    				            data = eval("("+ data + ")");
//    						 	eventList=data;
//    				           App.eventStore.loadData(eventList);
//    					 }
//    				});
//    			//this.onCancel();
//    		 }
...
//    },
    ...
    onDelete: function() {
    	var day=Ext.getCmp('date-range').getValue();
    	Ext.Ajax.request({
    		url : '../../arrange/delDayAttendCompanyCalendar.do',
    		params:{
    			day:day
    		},
    		 success : function(response, options){
