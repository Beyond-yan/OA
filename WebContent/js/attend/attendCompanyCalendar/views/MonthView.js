...
Ext.calendar.MonthView = Ext.extend(Ext.calendar.CalendarView, {
    ...
    getEventTemplate: function() {
        ...
        var dayHeight=(bodyHeight-180)/5;
        //if (!this.eventTpl) {
            var tpl,
            ...
            new Ext.XTemplate(
            '<div id="{_elId}" style="height:'+dayHeight+'px" class="{_selectorCls} {_colorCls} {values.spanCls} ext-cal-evt ext-cal-evr">',
            //body,
            '{Title}</div>'
            )
            : new Ext.XTemplate(
            '<div id="{_elId}" style="height:'+dayHeight+'px" class="{_selectorCls} {_colorCls} ext-cal-evt ext-cal-evr">',
            //body,
            '</div>'
            );
            tpl.compile();
    ...
    getTemplateEventData: function(evt) {
        ...
        var categoryNo=1;
        /*var configkey=evt[M.ConfigKey.name];
        var categoryNo="";
        if(evt[M.CalendarId.name]==3){
	        if(configkey=="newYear"){
	        	categoryNo=1;
	        }else if(configkey=="lunarNewYear"){
	        	categoryNo=2;
	        }else if(configkey=="qingMing"){
	        	categoryNo=3;
	        }else if(configkey=="mayDay"){
	        	categoryNo=4;
	        }else if(configkey=="dragonBoat"){
	        	categoryNo=5;
	        }else if(configkey=="midAutumn"){
	        	categoryNo=6;
	        }else if(configkey=="nationalDay"){
	        	categoryNo=7;
	        }
        }*/
        return Ext.applyIf({
            _selectorCls: selector,
            _colorCls: 'ext-color-' + (evt[M.CalendarId.name]?evt[M.CalendarId.name] : 'default')+ (evt._renderAsAllDay ? '-ad': ''),
            		//evt[M.CalendarId.name] : 'default')+categoryNo + (evt._renderAsAllDay ? '-ad': ''),
