...
SuggestBoxView = Ext.extend(Ext.Panel, {
	...
	initUIComponents : function() {
			//alert("this.typesss= "+Ext.util.JSON.decode(this.type) );
			//alert("this.dec= "+ this.type );
	...
	search : function(self) {
		if (self.searchPanel.getForm().isValid()) {// 如果合法
			...
			var baseParam = Ext.Ajax.serializeForm(self.searchPanel.getForm()
					.getEl());
			var deParams = Ext.urlDecode(baseParam);
			deParams.start = 0;
			deParams.limit = store.baseParams.limit;
			store.baseParams = deParams;
			self.gridPanel.getBottomToolbar().moveFirst();
		}
	},
