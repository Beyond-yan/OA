 ...
 * @description 拟稿管理
 */
ArchiveReportSend = Ext.extend(Ext.Panel, {
	// 条件搜索Panel
	searchPanel : null,
	// 数据展示Panel
	gridPanel : null,
	...
	constructor : function(_cfg) {
		...
		// 调用父类构造
		ArchiveReportSend.superclass.constructor.call(this, {
					id : 'ArchiveReportSend',
	...
	initUIComponents : function() {
		...
		this.searchPanel = new Ext.FormPanel({
			...
			region : 'north',
			frame : false,
			id : 'ArchiveReportSendSearchForm',
			layout : 'form',
			...
			items : [{
						...
						items : [{
									...
									handler : function() {
										var searchPanel = Ext
												.getCmp('ArchiveReportSendSearchForm');
		...
		var cm = new Ext.grid.ColumnModel({
			columns : [sm, new Ext.grid.RowNumberer(), {
						...
						dataIndex : 'subject',
							...
							var str = '<a href="#" style="text-decoration:none;color:#0358A3" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#0358A3\'"  onclick="ArchiveReportSend.edit('
									+ editId
									+ ','
									+ defId
									+ ',\''
						...
						renderer : function(value, metadata, record, rowIndex,
							...
							var flowName = record.data.recDepNames;
							var str = '';
							str += '<a href="#" style="text-decoration:none;color:#3D3D3D" title="发送公文" value=" " class="btn-archive-draft" onclick="ArchiveReportSend.edit('
									+ editId
									+ ','
									+ defId
									+ ',\''
					...
					if (btn == 'yes') {
						Ext.Ajax.request({
									...
									success : function(response, options) {
										...
										Ext.getCmp('ArchiveReportSendGrid').getStore()
												.reload();
									},
 ...
 * 修改
 */
ArchiveReportSend.edit = function(editId, defId, flowName) {
	new ArchiveReportSendView({
		archivesId:editId
	}).show();
}
/**
 ...
 *            ids
 */
ArchiveReportSend.remove = function(ids) {
	Ext.Msg.confirm('信息确认', '您确认要删除所选记录吗？', function(btn) {
		if (btn == 'yes') {
			Ext.Ajax.request({
				url : __ctxPath
				...
				success : function(response, options) {
					...
					Ext.getCmp('ArchiveReportSendGrid').getStore().reload();
				},
				failure : function(response, options) {
