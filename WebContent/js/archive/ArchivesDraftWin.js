	...
	init : function() {
		...
		this.toolbar = new Ext.Toolbar({
					...
					items : [{
								...
								scope : this,
								handler : ArchivesDraftWin.detail
							}]
				});
		...
		this.formPanel = new Ext.FormPanel({
			...
			items : [{
						...
						items : [{
									...
									items : [{
												...
												handler : function() {
													DepSelector.getView(
															function(depId,
																	depName) {
																Ext.getCmp('archivesWin.issueDep')
																		.setValue(depName);
																Ext.getCmp('archivesWin.depId')
																		.setValue(depId);
															}, true).show();
												}
											}]
									 ...
									 * 0px;margin-left:0px;', defaults : {
									 * border : false }, items : [{ xtype :
									 * 'label', style : 'padding:0px 0px 0px
									 * 0px;', text : '接收单位或部门', width : 105 }, { //
									 * columnWidth:.6, xtype : 'textarea', name :
									 * 'archives.recDepNames', width : '70%',
									 * readOnly : true, id :
									 * 'archivesWin.recDepNames' }, { xtype :
									 * 'hidden', name : 'archives.recDepIds', id :
									 * 'archivesWin.recDepIds' }, { xtype :
									 * 'button', iconCls : 'menu-department',
									 * text : '选择部门', handler : function() {
									 * DepSelector.getView( function(depIds,
									 * depNames) {
...
ArchivesDraftWin.getGridPanel = function(archivesId){
	return new Ext.grid.GridPanel({
		...
		store : new Ext.data.JsonStore({
			url : __ctxPath + '/archive/listArchivesDoc.do?archivesId='+archivesId,
			root : 'result',
			totalProperty : 'totalCounts',
			remoteSort : true,
			autoLoad:true,
		...
		columns : [new Ext.grid.RowNumberer(), {
					...
					renderer : function(v, m, r, i,c) {
						var str = '';
						if(v && v != 0){
							str = '<button title="查看" value=" " class="menu-archive-issue-manage" onclick="ArchivesDraftWin.detail('
								+ i + ')">&nbsp;&nbsp;</button>';
							str += '&nbsp;&nbsp;&nbsp;&nbsp;<button title="历史" value=" " class="btn-archive-history" onclick="ArchivesDraftWin.attach('
								+ i + ')">&nbsp;&nbsp;</button>';
							str += '&nbsp;&nbsp;&nbsp;&nbsp;<button title="编辑" value=" " class="btn-archive-copy" onclick="ArchivesDraftWin.edit('
								+ i + ')">&nbsp;&nbsp;</button>'	
						}
						return str;
					}
 ...
 * 查看公文附件
 */
ArchivesDraftWin.detail = function(rowIndex) {
	var store = Ext.getCmp("archiveDocGrid").getStore();
...
ArchivesDraftWin.attach = function(rowIndex) {
	var store = Ext.getCmp("archiveDocGrid").getStore();
	var record = store.getAt(rowIndex);
...
ArchivesDraftWin.edit = function(rowIndex) {
	var store = Ext.getCmp("archiveDocGrid").getStore();
	var record = store.getAt(rowIndex);
