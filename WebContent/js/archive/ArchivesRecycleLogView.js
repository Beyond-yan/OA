ArchivesRecycleLogView = Ext.extend(Ext.Panel, {
	// 条件搜索Panel
	searchPanel : null,
	// 数据展示Panel
	gridPanel : null,
	...
	constructor : function(_cfg) {
		...
		// 调用父类构造
		ArchivesRecycleLogView.superclass.constructor.call(this, {
					id : 'ArchivesRecycleLogView',
					iconCls : 'menu-recyle',
					title : '发文回收站',
					region : 'center',
					layout : 'border',
	...
	initUIComponents : function() {
		...
		var cm = new Ext.grid.ColumnModel({
			columns : [sm, new Ext.grid.RowNumberer({
						...
						renderer : function(value, metadata, record, rowIndex,colIndex){
							...
							var str = '<a href="#" title="'+value+'"'+' style="text-decoration:none;color:#0358A3" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#0358A3\'" onclick="ArchivesRecycleLogView.detail('+record.data.archives.archivesId+','+record.data.processRun.runId+','+record.data.processRun.proDefinition.defId+')">'+value+'</a>';
							return str;
						}
					}, {
						header : '删除人',
						...
						renderer : function(value, metadata, record, rowIndex,colIndex) {
							...
							if(record.data.logType==3){
							 str =  '<a href="#" style="text-decoration:none;color:#3D3D3D" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#3D3D3D\'" onclick="ArchivesRecycleLogView.recover('
									+record.data.id+')"> <img src="'+__ctxPath +'/images/btn/flow/changeTask.png" />恢复</a>&nbsp;';
							str +=  '<a href="#" style="text-decoration:none;color:#3D3D3D" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#3D3D3D\'" onclick="ArchivesRecycleLogView.multiDel('
		...
		this.topbar.add(new Ext.Button({
			...
			xtype : 'button',
			handler : function() {
				ArchivesRecycleLogView.multiDel("");
			}
		}));
		this.topbar.add(new Ext.Button({
			...
			xtype : 'button',
			handler : function() {
				ArchivesRecycleLogView.recover("");
			}
		}));
 ...
 *            editId
 */
ArchivesRecycleLogView.detail = function(editId,runId,defId) {
	var isGranted=false;
 ...
 * 恢复流程  
 */
ArchivesRecycleLogView.recover = function(logIds) {
	Ext.Msg.confirm('信息确认', '您确认要对该流程进行恢复吗？', function(btn) {
				if (btn == 'yes') {
					if(logIds==""){
						var grid = Ext.getCmp("ArchivesRecycleLogGrid");
 ...
 * 彻底删除
 */
ArchivesRecycleLogView.multiDel = function(logIds) {
