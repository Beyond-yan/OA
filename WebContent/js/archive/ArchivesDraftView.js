...
ArchivesDraftView = Ext.extend(Ext.Panel, {
	...
	addFlowStartPanel:function(topPanel){
		...
		Ext.Ajax.request({
			scope : this,
			url : __ctxPath+ '/archive/getFlowArchFlowConf.do?flowType=' + 0,
						...
						success:function(resp,options){
							...
							if (archivesId != null && archivesId != 'undefined') {
								panel.formPanel.getForm().load({
									...
									url : __ctxPath + '/archive/getIssueArchives.do?archivesId='+ archivesId,
									waitMsg : '正在载入数据...',
									success : function(form, action) {
										var archiveDocGrid = Ext.getCmp('archiveDocGrid');
										if(archiveDocGrid){
											archiveDocGrid.getStore().proxy.conn.url=__ctxPath+ '/archive/listArchivesDoc.do?archivesId='+archivesId
											archiveDocGrid.getStore().load();
										}
									},
