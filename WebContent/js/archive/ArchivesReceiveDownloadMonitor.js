/**
 * @author:Ropen
 * @class ArchivesReceiveDownloadMonitor
 * @extends Ext.Panel
 * @description 拟稿管理
 */
ArchivesReceiveDownloadMonitor = Ext.extend(Ext.Panel, {
	// 条件搜索Panel
	searchPanel : null,
	// 数据展示Panel
	gridPanel : null,
	...
	constructor : function(_cfg) {
		...
		// 调用父类构造
		ArchivesReceiveDownloadMonitor.superclass.constructor.call(this, {
					id : 'ArchivesReceiveDownloadMonitor',
	...
	initUIComponents : function() {
		...
		this.searchPanel = new Ext.FormPanel({
			...
			region : 'north',
			frame : false,
			id : 'ArchivesReceiveDownloadMonitorSearchForm',
			layout : 'form',
			defaults : {
				border : false
			},
			...
			items : [{
						...
						items : [{
									...
									handler : function() {
										var searchPanel = Ext
												.getCmp('ArchivesReceiveDownloadMonitorSearchForm');
		...
		this.store = new Ext.data.JsonStore({
			...
			+ "/system/listReceiveDownloadSysDataTransfer.do",
					...
					remoteSort : true,
					fields : [ 'archivesId','runid', 'archivesno', 'id',
								'subject', 'transferType','dataSource', 
								'createDate', 'archtype','fromSchema','createUserFullName']
				});
		// 加载数据
		this.store.load({
					params : {
		...
		var cm = new Ext.grid.ColumnModel({
			columns : [sm, new Ext.grid.RowNumberer({
						...
						renderer : function(value, metadata, record, rowIndex,
							...
							var str = '<a href="#" style="text-decoration:none;color:#0358A3" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#0358A3\'" onclick="ArchivesReceiveDownloadMonitor.detail('
								+ record.data.id+','+
		...
		this.gridPanel = new Ext.grid.GridPanel({
					id : 'ArchivesReceiveDownloadMonitorGrid',
					region : 'center',
					stripeRows : true,
					// tbar : this.topbar,
					store : this.store,
	...
	onRowAction : function(grid, record, action, row, col) {
		switch (action) {
			...
			case 'btn-flowView' :
				ArchivesReceiveDownloadMonitor.proDetail(record.data.archivesId);
				break;
			default :
				break;
		}
 ...
 *            record
 */
ArchivesReceiveDownloadMonitor.detail= function(archivesId,runid,dataSource,fromSchema) {
 ...
 * 
 */
ArchivesReceiveDownloadMonitor.proDetail = function(archiveId) {
