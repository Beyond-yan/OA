/**
 * @author:Ropen
 * @class ArchivesIssueSearch
 * @extends Ext.Panel
 * @description 拟稿管理
 */
ArchivesIssueSearch = Ext.extend(Ext.Panel, {
	// 条件搜索Panel
	searchPanel : null,
	// 数据展示Panel
	gridPanel : null,
	...
	initUIComponents : function() {
		...
		this.searchPanel = new Ext.FormPanel({
			...
			items : [{
				...
				items : [{
							...
							}*/new TreeSelector('depTreeSelector5', _url, '拟稿部门',
									'orgdepId', true), {
								name : 'startDate',
								xtype : 'hidden',
							...
							items : [{
								...
								listeners : {
									select : function() {
										Ext
												.getCmp('sentStoredStartDt')
												.setValue((this.getValue() + '-01-01'));
										Ext
												.getCmp('sentStoredendDate')
				...
				items : [{
							...
							iconCls : 'reset',
							handler : function() {
								Ext.getCmp('SentStoreSearchForm').getForm()
										.reset();
				...
				xtype : 'hidden',
				id : 'ArchivesIssueSearch.dir',
				value : 'DESC'
			}, {
				xtype : 'hidden',
				id : 'ArchivesIssueSearch.sort',
				value : 'archiveId'
			}, {
				xtype : 'hidden',
				name : 'orgdepId',
		...
		this.store = new Ext.data.JsonStore({
					...
					fields : ['archiveId', 'archivesNo', 'runSubject',
							'archCreateTime', 'signDate', 'defId', 'runId','flowName']
				});
		...
		var cm = new Ext.grid.ColumnModel({
			columns : [new Ext.grid.RowNumberer({
						...
						renderer : function(value, metadata, record, rowIndex,
								colIndex) {
							var str = '<a href="#" title="'+value+'"'+' style="text-decoration:none;color:#0358A3" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#0358A3\'" onclick=ArchivesIssueSearch.detail('
		...
		this.gridPanel = new Ext.grid.GridPanel({
					...
					bbar : new Ext.PagingToolbar({
								...
								if(sort != Ext.getCmp('ArchivesIssueSearch.sort').getValue()){
									dir = '';
								}
								Ext.getCmp('ArchivesIssueSearch.sort').setValue(sort);
								if('ASC' == dir){
									dir = 'DESC';
									Ext.getCmp('ArchivesIssueSearch.dir').setValue(dir);
								}else{
									dir = 'ASC';
									Ext.getCmp('ArchivesIssueSearch.dir').setValue(dir);
								}
	...
	tableToExcel : function(self) {
		...
		if (recordCount > 0) {
			...
			var url = __ctxPath
					...
					+ '&startDate='
					+ Ext.getCmp('sentStoredStartDt')
									.getValue()
					+ '&endDate='
					+Ext.getCmp('sentStoredendDate')
									.getValue() 
					+ '&subject='
					+ Ext.getCmp('sentStoredsubject').getValue()
					+'&orgdepId='
					...
					+'&dir='
					+Ext.getCmp('ArchivesIssueSearch.dir').getValue()
					+'&defId='
					+Ext.getCmp('sendDefId1').getValue()
					+'&sort='
					+Ext.getCmp('ArchivesIssueSearch.sort').getValue();
			url = encodeURI(url);
			url = encodeURI(url);
	...
	onRowAction : function(grid, record, action, row, col) {
		switch (action) {
			case 'btn-archives-detail' :
				ArchivesIssueSearch.detail(record.data.archiveId,
						record.data.runId, record.data.defId,record.data.flowName);
				break;
			default :
				break;
		}
...
ArchivesIssueSearch.detail = function(editId, runId, defId,flowName) {
	var isGranted=false;
	...
	}else if(flowName1s.indexOf(flowName)!=-1){
		new WfbaDetailView({
			archivesId : editId,
			runId : runId,
			defId : defId,
