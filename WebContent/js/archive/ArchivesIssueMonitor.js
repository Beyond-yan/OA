 ...
 * @description 拟稿管理
 */
ArchivesIssueMonitor = Ext.extend(Ext.Panel, {
	// 条件搜索Panel
	searchPanel : null,
	// 数据展示Panel
	gridPanel : null,
	...
	constructor : function(_cfg) {
		...
		// 调用父类构造
		ArchivesIssueMonitor.superclass.constructor.call(this, {
	...
	initUIComponents : function() {
		...
		this.searchPanel = new Ext.FormPanel({
			...
			region : 'north',
			frame : false,
			id : 'ArchivesIssueMonitorSearchForm',
			layout : 'form',
			...
			items : [{
						...
						items : [/*{
							...
							}*/new TreeSelector('depTreeSelector4', _url, '拟稿部门',
									'issuerDepId', true)]
						},{
							columnWidth : .3,
							layout : 'form',
						...
						items : [{
									...
									handler : function() {
										var searchPanel = Ext
												.getCmp('ArchivesIssueMonitorSearchForm');
		...
		var cm = new Ext.grid.ColumnModel({
			columns : [sm, new Ext.grid.RowNumberer({
						...
						renderer : function(value, metadata, record, rowIndex,
							 ...
							 str += '<a href="#" style="text-decoration:none;color:#0358A3" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#0358A3\'" onclick=ArchivesIssueMonitor.detail('
						...
						renderer : function(value, metadata, record, rowIndex,
							...
							if (isGranted('_ArchiveIssueEdit')) {
								str += '<a href="#" style="text-decoration:none;color:#3D3D3D" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#3D3D3D\'" onclick="ArchivesIssueMonitor.edit('
										+ record.data.taskId
										+ ',\''
							...
							if (isGranted('_ArchiveIssueDel')) {
								if (curUserInfo.isAdmin || curUserInfo.iscommonAdmin
									...
									str += '<a href="#" style="text-decoration:none;color:#3D3D3D" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#3D3D3D\'" onclick="ArchivesIssueMonitor.stopProcess('
											+ record.data.archiveId
											+ ',\''
											+ record.data.piid
											+ '\')"><img src="'
...
//										&& taskName.toString().indexOf("会签") == -1
									...
									if (taskName != "重新发起"&&preActivityname!="退回"&&preActivityname!="撤回"&&preActivityname!="流程编辑") {
										str += '<a href="#" style="text-decoration:none;color:#3D3D3D" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#3D3D3D\'" onclick="ArchivesIssueMonitor.backProcess('
												+ record.data.taskId
												+ ',\''
												+ record.data.activityname
												+ '\',\''
							...
							str += '<a href="#" style="text-decoration:none;color:#3D3D3D" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#3D3D3D\'" onclick="ArchivesIssueMonitor.proDetail('
								+ record.data.runId
								+ ','
								+ record.data.defId
								+ ',\''
	...
	onRowAction : function(grid, record, action, row, col) {
		switch (action) {
			case 'btn-archives-detail' :
				ArchivesIssueMonitor.detail(record.data.archiveId,
						record.data.taskId, record.data.defId,record.data.flowName);
				break;
			case 'btn-flowView' :
				ArchivesIssueMonitor.proDetail(record.data.runId,
						record.data.defId, record.data.piid,
						record.data.runSubject);
				break;
			case 'btn-edit' :
				ArchivesIssueMonitor.edit(record.data.taskId,
						record.data.activityname);
				break;
			case 'menu-holiday' :
				ArchivesIssueMonitor.stopProcess(record.data.archiveId,
						record.data.piid);
				break;
			default :
				break;
 ...
 *            editId
 */
ArchivesIssueMonitor.detail = function(editId, runId, defId,flowName) {
	var isGranted=false;
	if(curUserInfo.isOfficeStaffRole||curUserInfo.isAdmin||curUserInfo.isArchivesManager)
		{
		  isGranted=true;
		}
	var flowNames="采购申请处理笺,"
				 +"外出登记处理笺,"
				 +"外业申请处理笺,"
				 +"休假申请处理笺,"
				 +"用车申请处理笺,"
	...
	}else if(flowName && flowName.indexOf("委发规范性文件备案材料移交")!=-1){
		new WfbaDetailView({
			archivesId : editId,
			runId : runId,
			defId : defId,
 ...
 * 
 */
ArchivesIssueMonitor.edit = function(taskId, activityname) {
 ...
 *            editId
 */
ArchivesIssueMonitor.subDetail = function(editId) {
	new ArchSubDetailForm({
				parentArchId : editId
			});
}

ArchivesIssueMonitor.stopProcess = function(editId, piId) {
 ...
 *            name
 */
ArchivesIssueMonitor.proDetail = function(runId, defId, piId, name) {
 ...
 * @param {}
 */
ArchivesIssueMonitor.backProcess = function(taskId, activityName,
		preActivityname, preUserId) {
	Ext.Msg.confirm('提示', '确定撤回该流程？', function(btn, text) {
		if (btn == 'yes') {
			// 撤回流程
			var formPanel = Ext.getCmp('ArchivesIssueMonitorSearchForm');
								...
								if(prevDestName.toString().indexOf("会签") == -1){
									if(activityName.toString().indexOf("会签")==-1){
										formPanel.getForm().submit({
											url : __ctxPath
													+ "/flow/nextForSyncProcessActivity.do",
											method : 'post',
											waitMsg : '正在提交处理，请稍等',
                                            timeout:120000,
											scope : this,
