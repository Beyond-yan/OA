/**
 * @class JWArchivesHandleView
 * @extends Ext.Panel
 * @description [ArchivesHandle]管理
 */
JWArchivesHandleView = Ext.extend(Ext.Panel, {
	// 条件搜索Panel
	searchPanel : null,
	// 数据展示Panel
	gridPanel : null,
	...
	constructor : function(_cfg) {
		...
		// 调用父类构造
		JWArchivesHandleView.superclass.constructor.call(this, {
					id : 'JWArchivesHandleView',
	...
	initUIComponents : function() {
		...
		this.searchPanel = new Ext.FormPanel({
				...
				xtype : 'hidden',
				id : 'JWArchivesHandleView.dir',
				value : 'DESC'
			}, {
				xtype : 'hidden',
				id : 'JWArchivesHandleView.sort',
				value : 'archiveId'
			}]
		...
		this.store = new Ext.data.JsonStore({
			...
			fields : [{
					...
					'isReply', 'archivesNo', 'orgdepName', 'issuedep','curDepId','curDepName','flowName','dataValue','piid','creatorDepName','creatorDepId','depSignNo','issueDate','writtenDate']
		});
		// 加载数据
		this.store.load({
					params : {
		...
		var cm = new Ext.grid.ColumnModel({
			columns : [ new Ext.grid.RowNumberer({
						...
						renderer : function(value, metadata, record, rowIndex,colIndex){
							var str = '<a href="#" title="'+value+'"'+' style="text-decoration:none;color:#0358A3" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#0358A3\'" onclick="JWArchivesHandleView.detail('+record.data.archiveId+','+record.data.runId+','+record.data.defId+')">'+value+'</a>';
							return str;
						}
					}, {
						header : '收文编号',
		...
		this.gridPanel = new Ext.grid.GridPanel({
					...
					bbar : new Ext.PagingToolbar({
								...
								if(sort != Ext.getCmp('JWArchivesHandleView.sort').getValue()){
									dir = '';
								}
								Ext.getCmp('JWArchivesHandleView.sort').setValue(sort);
								if('ASC' == dir){
									dir = 'DESC';
									Ext.getCmp('JWArchivesHandleView.dir').setValue(dir);
								}else{
									dir = 'ASC';
									Ext.getCmp('JWArchivesHandleView.dir').setValue(dir);
								}
...
//行的Action
	onRowAction : function(grid, record, action, row, col) {
		switch (action) {
			case 'btn-archives-detail' :
				JWArchivesHandleView.detail(record.data.archiveId,record.data.runId,record.data.defId);
				break;
			default :
				break;
		}
	...
	//把表格导出到Excel
					...
					+ '&startDate='
					+ Ext.util.Format.date(Ext.getCmp('receiveStoredStartDt')
									.getValue(), 'Y-m-d') 
					+'&endDate='
					+Ext.util.Format.date(Ext.getCmp('RsentStoredendDate')
									.getValue(), 'Y-m-d') 
					+ '&subject='
					...
					+'&dir='
					+Ext.getCmp('JWArchivesHandleView.dir').getValue()
					+'&sort='
					+Ext.getCmp('JWArchivesHandleView.sort').getValue();
					url = encodeURI(url);
					url = encodeURI(url);
 ...
 *            editId
 */
JWArchivesHandleView.detail = function(editId,runId,defId) {
	var isGranted=false;
