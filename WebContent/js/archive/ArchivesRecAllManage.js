/**
 * @author:Ropen
 * @class ArchivesRecAllManage
 * @extends Ext.Panel
 * @description 拟稿管理
 */
ArchivesRecAllManage = Ext.extend(Ext.Panel, {
	// 条件搜索Panel
	searchPanel : null,
	// 数据展示Panel
	gridPanel : null,
	...
	constructor : function(_cfg) {
		...
		// 调用父类构造
		ArchivesRecAllManage.superclass.constructor.call(this, {
					id : 'ArchivesRecAllManage',
	...
	initUIComponents : function() {
		...
		this.searchPanel = new Ext.FormPanel({
			...
			region : 'north',
			frame : false,
			id : 'ArchivesRecAllManageSearchForm',
			layout : 'form',
			defaults : {
				border : false
			},
			...
			items : [{
						...
						items : [{
							...
							}*/new TreeSelector('depTreeSelector6', _url, '主办部门',
									'issuerDepId', true)]
						},  {
							columnWidth : .3,
							layout : 'form',
						...
						items : [{
									...
									handler : function() {
										var searchPanel = Ext
												.getCmp('ArchivesRecAllManageSearchForm');
		...
		var cm = new Ext.grid.ColumnModel({
			columns : [sm, new Ext.grid.RowNumberer({
						...
						renderer : function(value, metadata, record, rowIndex,
								colIndex) {
							var str = '<a href="#" style="text-decoration:none;color:#0358A3" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#0358A3\'" onclick="ArchivesRecAllManage.detail('
	...
	onRowAction : function(grid, record, action, row, col) {
		switch (action) {
			case 'btn-archives-detail' :
				ArchivesRecAllManage.detail(record.data.archiveId,
						record.data.runId, record.data.defId);
				break;
			case 'btn-flowView' :
				ArchivesRecAllManage.proDetail(record.data.runId,
						record.data.defId, record.data.piid,
						record.data.runSubject);
				break;
			case 'menu-archive-template' :
				ArchivesRecAllManage.ArchivesShareForm(record.data.archiveId);
				break;
			default :
				break;
		}
 ...
 *            record
 */
ArchivesRecAllManage.detail = function(editId, runId, defId) {
	var isGranted=false;
 ...
 * 
 */
ArchivesRecAllManage.proDetail = function(runId, defId, piId, name) {
...
ArchivesRecAllManage.ArchivesShareForm = function(editId) {
