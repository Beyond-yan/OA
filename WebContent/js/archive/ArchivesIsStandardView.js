ArchivesIsStandardView = Ext.extend(Ext.Panel, {
	// 条件搜索Panel
	searchPanel : null,
	// 数据展示Panel
	gridPanel : null,
	...
	constructor : function(_cfg) {
		...
		// 调用父类构造
		ArchivesIsStandardView.superclass.constructor.call(this, {
					id : 'ArchivesIsStandardView',
	...
	initUIComponents : function() {
		...
		this.searchPanel = new Ext.FormPanel({
			height : 88,
			region : 'north',
			id : 'isStandardSentStoreSearchForm',
			layout : 'form',
			...
			items : [{
				...
				items : [{
							...
							items : [{
										fieldLabel : '标题',
										id : 'isStandardsentStoredsubject',
										name : 'subject',
										xtype : 'textfield'
									}]
						}, {
							...
							items : [{
								...
								listeners : {
									select : function() {
										Ext
												.getCmp('isStandardsentStoredStartDt')
												.setValue((this.getValue() + '-01-01'));
										Ext
												.getCmp('isStandardsentStoredendDate')
							...
							items : [{
										width : 60,
										id : 'isStandardsentStoredArchivesNo',
										fieldLabel : '发文编号',
										name : 'archiveNo',
										xtype : 'textfield'
									}]
							...
							items : [{
								width : 60,
								id : 'isStandardsentStoredsnConfigId',
								fieldLabel : '编号办法',
				...
				items : [{
							...
							iconCls : 'reset',
							handler : function() {
								Ext.getCmp('isStandardSentStoreSearchForm').getForm()
										.reset();
				...
				xtype : 'hidden',
				name : 'endDate',
				id : 'isStandardsentStoredendDate'
			}]
		});// end of the searchPanel

		// 加载数据至store
		this.store = new Ext.data.JsonStore({
					url : __ctxPath + "/archive/listIsStandardSentStoreArchives.do",
					baseParams : {
						archiveType : 0
					},
					root : 'result',
		...
		var cm = new Ext.grid.ColumnModel({
			columns : [sm, new Ext.grid.RowNumberer({
						...
						renderer : function(value, metadata, record, rowIndex,
								colIndex) {
							var str = '<a href="#" style="text-decoration:none;color:#0358A3" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#0358A3\'" onclick="ArchivesIsStandardView.detailT('
...
/*	tableToExcel : function(self) {
		...
		if (recordCount > 0) {
			 ...
			 * Ext.getCmp('isStandardSentStoreSearchForm').getForm().submit({ url :
			...
			var url = __ctxPath
					...
					+ '&archiveNo='
					+ Ext.getCmp('isStandardsentStoredArchivesNo').getValue()
					+ '&startDate='
					+ Ext.util.Format.date(Ext.getCmp('isStandardsentStoredStartDt')
									.getValue(), 'Y-m-d')
					+ '&endDate='
					+ Ext.util.Format.date(Ext.getCmp('isStandardsentStoredendDate')
									.getValue(), 'Y-m-d') 
					+ '&subject='
					+ Ext.getCmp('isStandardsentStoredsubject').getValue()
					+ '&snConfigId='
					+ Ext.getCmp('isStandardsentStoredsnConfigId').getValue();
			url = encodeURI(url);
			url = encodeURI(url);
	...
	onRowAction : function(grid, record, action, row, col) {
		switch (action) {
			case 'btn-archives-detail' :
				ArchivesIsStandardView.detail(record.data.archiveId);
				break;
			case 'btn-flowView' :
				ArchivesIsStandardView.toUnStandard(record.data.archiveId);
				break;
			default :
				break;
		}
...
ArchivesIsStandardView.detail = function(editId) {
	var grid=Ext.getCmp('isStandardArchivesGrid');
...
ArchivesIsStandardView.toUnStandard=function(archivesId){
...
ArchivesIsStandardView.detailT = function(editId, runId, defId) {
	new ArchDetailView({
				archivesId : editId,
				runId : runId,
				defId : defId,
