/**
 * @author:Ropen
 * @class ArchivesDraftAllManage
 * @extends Ext.Panel
 * @description 拟稿管理
 */
ArchivesDraftAllManage = Ext.extend(Ext.Panel, {
	// 条件搜索Panel
	searchPanel : null,
	// 数据展示Panel
	gridPanel : null,
	...
	constructor : function(_cfg) {
		...
		// 调用父类构造
		ArchivesDraftAllManage.superclass.constructor.call(this, {
					id : 'ArchivesDraftAllManage',
	...
	initUIComponents : function() {
		...
		this.searchPanel = new Ext.FormPanel({
			...
			frame : false,
			border : false,
			id : 'ArchivesDraftAllManageSearchForm',
			layout : 'form',
			defaults : {
				border : false
			},
			...
			items : [{
						...
						items : [/*{
							...
							}*/new TreeSelector('depTreeSelector1', _url, '拟稿部门',
									'orgDepId', true)]
						}, {
							columnWidth : .3,
							layout : 'form',
						...
						items : [{
									...
									handler : function() {
										var searchPanel = Ext
												.getCmp('ArchivesDraftAllManageSearchForm');
		...
		var cm = new Ext.grid.ColumnModel({
			columns : [sm, new Ext.grid.RowNumberer({
						...
						renderer : function(value, metadata, record, rowIndex,
							...
							var str = '<a href="#" title="'+value+'"'+' style="text-decoration:none;color:#0358A3" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#0358A3\'" onclick="ArchivesDraftAllManage.detail('
						...
						renderer : function(value, metadata, record, rowIndex,
							...
							var str = '';
							str += '<a href="#" style="text-decoration:none;color:#3D3D3D" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#3D3D3D\'" onclick="ArchivesDraftAllManage.proDetail('
									+ record.data.runId
									+ ','
									+ record.data.defId
									+ ',\''
							...
							if (isGranted('_ArchiveRecAllEdit')) {
								str += '<a href="#" style="text-decoration:none;color:#3D3D3D" onMouseOver = "this.style.color =\'red\'" onMouseOut = "this.style.color=\'#3D3D3D\'" onclick="ArchivesDraftAllManage.edit('
										+ record.data.taskId
										+ ',\''
	 ...
	 * ArchivesDraftAllManage.detail(record.data.archiveId,record.data.taskId,record.data.defId);
	 * break; case 'btn-flowView' :
	 * ArchivesDraftAllManage.proDetail(record.data.runId,record.data.defId,record.data.piid,record.data.runSubject);
	 * break; case 'btn-edit' :
	 * ArchivesDraftAllManage.edit(record.data.taskId,record.data.activityname);
	 * break; case 'menu-holiday' :
	 * ArchivesDraftAllManage.stopProcess(record.data.archiveId,record.data.piid);
	 * break; default : break; } },
	 */
	/**
	 * 查阅详情
 ...
 *            editId
 */
ArchivesDraftAllManage.detail = function(editId, runId, defId) {
	var isGranted=false;
 ...
 * 
 */
ArchivesDraftAllManage.edit = function(taskId, activityname) {
 ...
 *            editId
 */
ArchivesDraftAllManage.subDetail = function(editId) {
	new ArchSubDetailForm({
				parentArchId : editId
			});
}

ArchivesDraftAllManage.stopProcess = function(editId, piId) {
				...
				if (btn == 'yes') {
					...
					Ext.Ajax.request({
								...
								success : function(resp, options) {
									...
									Ext.getCmp('ArchivesDraftAllManageGrid')
											.getStore().reload()
								},
 ...
 *            name
 */
ArchivesDraftAllManage.proDetail = function(runId, defId, piId, name) {
