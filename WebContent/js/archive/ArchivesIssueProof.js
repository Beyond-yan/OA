...
/**
 * @author:Ropen
 * @class ArchivesIssueProof
 * @extends Ext.Panel
 * @description 拟稿管理
 */
ArchivesIssueProof = Ext.extend(Ext.Panel, {
	// 条件搜索Panel
	searchPanel : null,
	// 数据展示Panel
	gridPanel : null,
			...
			columns : [sm, new Ext.grid.RowNumberer(), {
						...
						renderer : function(value) {
							if (value == 4) {
								...
								return '<font color="green">已复核</font>';
							}
						}
					}, {
						header : '秘密等级',
						...
						renderer : function(value, metadata, record, rowIndex,
							...
							var str = '';
							str += '<button title="查阅详情" value=" " class="btn-archives-detail" onclick="ArchivesIssueProof.detail('
									+ editId + ')">&nbsp;&nbsp;</button>';
							if (status == '5'
									&& isGranted('_ArchivesIssueProofEdit')) {// 只有未校对的公文才能管理附件
								str += '<button title="管理附件" value=" " class="btn-archive-attachment" onclick="ArchivesIssueProof.attach('
										+ editId + ')">&nbsp;&nbsp;</button>';
							}
							return str;
						}
 ...
 *            editId
 */
ArchivesIssueProof.detail = function(editId) {
 ...
 *            editId
 */
ArchivesIssueProof.attach = function(editId) {
	new ArchivesDocWin({
				archivesId : editId
			}).show();
}
