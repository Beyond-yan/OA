...
function right(mainStr,lngLen) { 
	...
	function $deldetailRs(conf){
		...
		$postdetailDel(params);
	};
	
	function $postdetailDel(conf){
	...
	function $postSaveForm(conf){
		if(conf.formPanel.getForm().isValid()){
			var ids=$getGdSelectedIds(conf.gridPanel,conf.idName);
			//conf.params.ids = ids;
...
DiningBookingFormDetail = Ext.extend(Ext.Window, {
	// 构造函数
	...
	constructor : function(_cfg) {
		...
		DiningBookingFormDetail.superclass.constructor.call(this, {
			id : 'DiningBookingFormDetailWin',
			layout : 'form',
		...
		var foodtypestore = new Ext.data.SimpleStore(
				{
					...
					listeners : {
						load : function() {
								Ext.getCmp('DBFDtlfoodtypeid')
									.setValue(Ext.getCmp('DBFDtlfoodtypeid').getValue());
						}
					}									
				});		
		...
		var diningDetailstore = new Ext.data.Store(
				{
					...
					reader:new Ext.data.JsonReader({
						...
						fields : [ 'id','appUser.fullname','diningFoodtype.foodtypename',
						           'qty','diningFoodtype.price','amount','diningroom' ]
					}),	
					remoteSort : false
				});				
...
//			}
	...
	createRs:function() {
		$postSaveForm( {
			...
			scope : this,
			idName: 'userId',
			url : __ctxPath + '/diningMgnt/addDetailDiningBooking.do?diningroom='
					+encodeURIComponent(Ext.getCmp('DiningBookingFrmDtldiningroom').getValue())
					+'&foodtypeid='+Ext.getCmp('DBFDtlfoodtypeid').getValue(),
