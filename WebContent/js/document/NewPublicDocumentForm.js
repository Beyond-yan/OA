...
function removeFormFile(obj, fileId) {
	var fileIds = Ext.getCmp('NewPublicDocumentForm.fileIdsB');
...
NewPublicDocumentForm.prototype.setup = function(dId, dName) {
	...
	var formPanel = new Ext.FormPanel({
		...
		layout : 'form',
		id : 'NewPublicDocumentFormPanelA',
		autoScroll : true,
		border : false,
		...
		items : [{
					...
					items : [{
								...
								items : [{
											fieldLabel : '附件',
											xtype : 'panel',
											id : 'NewPublicDocumentForm.filePanelC',
											frame : false,
											height : 80,
											autoScroll : true,
											html : ''
								...
								items : [{
									...
									handler : function() {
										var dialog = App.createUploadDialog({
											...
											callback : function(data) {
												var fileIds = Ext
														.getCmp('NewPublicDocumentForm.fileIdsB');
												var filePanel = Ext
														.getCmp('NewPublicDocumentForm.filePanelC');
									...
									xtype : 'hidden',
									id : 'NewPublicDocumentForm.fileIdsB',
									name : 'fileIds'
								}]
							}]
				}]
	...
	if (this.docId != null && this.docId != 'undefined') {
		formPanel.getForm().load({
			...
			success : function(form, action) {
				...
				var filePanel = Ext.getCmp('NewPublicDocumentForm.filePanelC');
				var fileIds = Ext.getCmp('NewPublicDocumentForm.fileIdsB');
...
NewPublicDocumentForm.prototype.initToolbar = function() {
	...
	var toolbar = new Ext.Toolbar({
		...
		items : [{
			...
			iconCls : 'btn-save',
			handler : function() {
				var content = Ext.getCmp('NewPublicDocumentForm.content')
						.getValue();
				...
				var docForm = Ext.getCmp('NewPublicDocumentFormPanelA');
				if (docForm.getForm().isValid()) {
					docForm.getForm().submit({
						...
						success : function(docForm, o) {
							...
							var docForm = Ext
									.getCmp('NewPublicDocumentFormPanelA');
							...
							tabPanel
									.remove(Ext.getCmp('NewPublicDocumentForm'));
						},
			...
			iconCls : 'reset',
			handler : function() {
				Ext.getCmp('NewPublicDocumentForm.docFolderName').reset();
				Ext.getCmp('NewPublicDocumentForm.docName').reset();
				var fileAttaches = Ext.getCmp("NewPublicDocumentForm.fileIdsB");
				var filePanel = Ext.getCmp('NewPublicDocumentForm.filePanelC');
				filePanel.body.update('');
				fileAttaches.setValue('');
			}
