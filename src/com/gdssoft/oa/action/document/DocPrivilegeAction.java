...
public class DocPrivilegeAction extends BaseAction {
	...
	public String change() {
		...
		if (StringUtils.isNotEmpty(strPrivilegeId)) {
			docPrivilege = docPrivilegeService.get(Long
			...
			if (in > 0) {
				...
				if ("rightU".equals(strField)) {
					StringBuffer newBuff = new StringBuffer();
					if ("true".equals(strFieldValue)) {
						newBuff.append(buff.charAt(0)).append("1").append(
								buff.charAt(2));
					} else {
						newBuff.append(buff.charAt(0)).append("0").append(
								buff.charAt(2));
					}
					rights = newBuff.toString();
				}
	...
	/**
	 * 添加权限 权限right是各个权限二进制合成 其中：rights=1 相应的二进制为：001 也即是表示只有读的权限 如此类推
	 * 
	 * @return
	 */
	public String add() {
			...
			if (StringUtils.isNotEmpty(strRoleIds)) {
				...
				if (roles.length > 0) {
					for (int i = 0; i < roles.length; i++) {
						...
						AppRole appRole = appRoleService
								.get(roleId.longValue());
