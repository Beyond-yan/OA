	...
	public String save() {
		...
		if (isPersonal != 1) {
			if (StringUtils.isNotEmpty(issueScopeIds)) {// 发布范围
				boolean b = planAttendService.deletePlanAttend(workPlan
						.getPlanId(), ISDEPARTMENT, NOTPRIMARY);
				if (b) {
			...
			if (StringUtils.isNotEmpty(participantsIds)) {// 参与人
				boolean b = planAttendService.deletePlanAttend(workPlan
						.getPlanId(), NOTDEPARTMENT, NOTPRIMARY);
				if (b) {
			...
			if (StringUtils.isNotEmpty(principalIds)) {// 负责人
				boolean b = planAttendService.deletePlanAttend(workPlan
						.getPlanId(), NOTDEPARTMENT, ISPRIMARY);
				if (b) {
